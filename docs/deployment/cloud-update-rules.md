...
### 方式一：本地一键推云端（推荐，跟随开发快速上线测试）

当前服务器 **未使用 Git**，代码通过「本机打包上传」更新。在**本机**项目根目录执行：

```powershell
.\scripts\deploy-to-cloud.ps1
```

- 自动完成：打包源码（排除 node_modules、dist、server/data、.env）→ 上传到服务器 `/opt/awcc` → 在服务器执行 `npm ci`、`npx prisma generate`、`npm run build`、数据库迁移（若有）、`pm2 restart awcc`。
- 前置条件：本机已配置免密 SSH（见上文「让自动化/助手获得与服务器交互能力」），密钥默认 `~/.ssh/awcc_deploy`。
- 完成后访问：https://ai.667788.cool

### 方式二：登录服务器后更新

1. SSH 登录：`ssh ubuntu@43.160.204.235`
2. 进入应用目录：`cd /opt/awcc`
3. 若服务器为 Git 仓库：`git pull`，然后执行 `USE_PM2=1 bash scripts/deploy-update.sh`
4. 若服务器非 Git 仓库：代码需先通过方式一或 rsync/tar 上传，再在服务器执行：
...
npx prisma generate
npm run build
npx prisma migrate deploy  # 若有新增迁移
pm2 restart awcc
```
...
- 因 SSH 需密码交互，无法在本机自动完成登录；请在本机执行一次 `ssh ubuntu@43.160.204.235` 登录后，在服务器上按「首次部署步骤」执行。
